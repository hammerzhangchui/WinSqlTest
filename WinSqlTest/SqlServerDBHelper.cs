...
namespace WinSqlTest
{
    class SqlServerDBHelper
    {
        ...
        public int ExecuteScalarInsert(string connectString,string commandStr)
        {
            ...
            if (err.Length > 0)
            {
                throw new Exception(err); //WebFaultException<SimpleException>(new SimpleException() { Message = err }, HttpStatusCode.InternalServerError);
            }
            else
            {
                return ret;
        ...
        public int ExecuteScalarInsert(string commandStr)
        {
            ...
            if (err.Length > 0)
            {
                throw new Exception(err); //<SimpleException>(new SimpleException() { Message = err }, HttpStatusCode.InternalServerError);
            }
            else
            {
                return ret;
