...
namespace WinSqlTest
{
    ...
    {
        /// <summary>
        /// 1=sqlserver, 2=mysql, 3=sqlite
        /// </summary>
        static int DBType = 1;
        public static void SetConnStr(string key)
        {
            connStrRead = ConfigurationManager.ConnectionStrings[key].ConnectionString;
            connStrWrite = ConfigurationManager.ConnectionStrings[key].ConnectionString;

            if (connStrRead.StartsWith("Data Source"))
            {
                DBType = 1;
            }
            else if (connStrRead.StartsWith("server="))
            {
                DBType = 2;
            }
            else if (connStrRead.EndsWith(".db"))
            {
                DBType = 3;
                connStrRead = string.Format("data source={0}", AppDomain.CurrentDomain.BaseDirectory + connStrRead + ";");
                connStrWrite = connStrRead;
            }
            else
            {
                DBType = 1;
        ...
        static string connStrRead = ConfigurationManager.ConnectionStrings["MySqlDBConnect"].ConnectionString;
        static string connStrWrite = ConfigurationManager.ConnectionStrings["MySqlDBConnect"].ConnectionString;
        ...
        public static IDbConnection GetConnection(bool useWriteConn)
        {
            ...
            {
                if (useWriteConn)
                    return new SQLiteConnection(connStrWrite);
                return new SQLiteConnection(connStrRead);
            }
            else
            {
                return null;
